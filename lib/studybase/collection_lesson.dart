...
class CollectionLesson extends StatefulWidget {
  final DocumentSnapshot doc;
  const CollectionLesson({super.key,
  required this.doc
  });
  @override
  State<CollectionLesson> createState() => _CollectionLessonState();
}

class _CollectionLessonState extends State<CollectionLesson> {
...
Future<void> stopListening() async {
 ...
 /*
  ...
  try {
    ...
    await FirebaseFirestore.instance.collection('users').doc(uid).collection('study in studio collection').add({
      'video ID':widget.doc['video ID'],
      'explanation':widget.doc['explanation'],
      'date':widget.doc['date'],
      'creator ID':widget.doc['uid'],
...
Future<void> createFlashcards() async {
  ...
  for (int i = 0; i < flashcards.length; i++) {
    await FirebaseFirestore.instance.collection('users').doc(uid).collection('my collection').doc(widget.doc.id).collection('flashcards').add({
      'answer':'', 
    ...
    return Scaffold(
      ...
      body: NestedScrollView(
        headerSliverBuilder: (context, innerBoxIsScrolled) => [
          SliverAppBar(
            ...
            actions: [
              ...
              TextButton.icon(
                icon: Icon(Icons.delete, color: Colors.red, size: 13,),
                onPressed: () async {
                  ...
                  try {                
                    ...
                    await FirebaseFirestore.instance.collection('users').doc(uid).collection('my collection').doc(widget.doc.id).delete();

                    Navigator.pop(context);
                    Navigator.pop(context);

                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Notes removed from collection.')));
        ...
        body: picked == 'LESSON'
            ...
            StreamBuilder(
              stream: FirebaseFirestore.instance.collection('users').doc(uid).collection('my collection').doc(widget.doc.id).collection('flashcards').snapshots(), 
              ...
              Padding(
                ...
                child: Text("YOU NEED AT LEAST 2 FLASHCARDS TO SHOWCASE THEM.",
                textAlign: TextAlign.center,
                style: GoogleFonts.viga(
                  color: br.black,
                  fontSize: 15
